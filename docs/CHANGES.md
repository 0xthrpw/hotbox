...
## Phase 5 — Volumes + command override for plain services (external monorepo support)

Unblocks running stateful dependencies (Postgres, Redis, Elasticsearch) as ordinary hotbox services so that GitHub-built app fleets (EFP `api-v2`, grails `backend`) can bring their own databases — managed siblings remain unavailable on github services, and that's now fine: declare a volume, wire a `DATABASE_URL` variable, done. Motivating deploys: EFP needs plain Postgres 15+/Redis; grails needs Postgres with `wal_level=logical` (hence the command override), Redis, and Elasticsearch 8.

### What shipped

**Schema (zod only — no DB migration).** `ServiceConfigSchema` gains `volumes` (max 8 of `{name, mountpoint, ro}`; slug-style names via a shared `ShortNameSchema` also used by `requires[].name` — trailing dashes now rejected for both; absolute mountpoints; duplicate name/mountpoint rejected), `command`, and `entrypoint` (exec-form argv, 1–64 elements; elements may be empty strings, matching what the tokenizer produces for bare quotes). Template services **reject** `config.command/entrypoint/volumes` (template roles own their spec; accepting them would persist inert values the reconciler never reads). `CreateDeploymentInputSchema` gains `command`/`entrypoint` as one-off overrides with the same semantics as its `env` field — not persisted to config, with the documented caveat that ingress-edit clones carry overrides forward until a plain redeploy (same as one-off env). The `deployments.volume_refs/command/entrypoint` columns and the `volumes` table already existed (init migration) and the jsonb write plugin already whitelisted them — this phase just makes them reachable. `VolumeRef` gains `name` (the docker volume name); this also fixes a latent bug where `buildOptionsForRole` passed `volume_id` (a UUID) as the docker Mount Source, which docker treats as a volume *name* — dead code until now since nothing populated volume_refs.

**Derivation model.** `services.config` is the durable declaration; every deployment snapshots it — the exact pattern variables→`env_snapshot` uses. `dockerVolumeName()` (in `packages/shared/src/naming.ts`, alongside the new `qualifiedServiceName()`) namespaces as `<project>-<env>-<service>_<name>` — the `_` separator (impossible in slugs) prevents dash-split aliasing between e.g. service `web` volume `db-data` and service `web-db` volume `data`. `resolveVolumeRefs()` (`apps/api/src/lib/volume-refs.ts`) idempotently upserts `volumes` rows in a single statement (`onConflict(host_id,name) doUpdateSet` + `RETURNING`) and **fails the deployment if the row belongs to a different service** — two services silently sharing a datadir is the one outcome never allowed. All five deployment-creation sites participate: service create (image branch snapshots refs+command; github branch pre-materialises volumes rows so errors surface at create time), the build worker (re-derives from config on every build — which is what makes the github *first* deploy work, where no previous deployment exists), explicit redeploy (re-derives; body command wins over config; command overrides rejected for template services), `redeployForConfigChange` (clones verbatim per its contract — an ingress edit can't drop mounts), and env duplicate (re-derives under the new env slug → fresh empty volumes, config-only by design).

**Reconciler.** `ensureDeploymentInfra` now ensures declared volumes exist (via `ensureVolume`, so they get the `hotbox.managed` label) before `applyPlan`. The non-template branch of `buildOptionsForRole` mounts by `volume_refs[].name` and passes `deployment.command`/`entrypoint` through (template roles keep their template-declared command). `aliasesFor` adds project-env-qualified DNS aliases (`<proj>-<env>-<slug>[-<role>]`, via `qualifiedServiceName`) alongside the short ones — the short `<slug>` alias collides on `hotbox-public` when two projects reuse a slug (docker DNS round-robins); the qualified form is unambiguous host-wide and is the recommended name for cross-service wiring (e.g. `DATABASE_URL=postgres://app:pw@efp-prod-efp-db:5432/app`). Qualified aliases longer than the 63-char DNS label limit are skipped (rather than risking the engine rejecting the container create); the short aliases keep working.

**Data safety.** Two layers: service create defaults `replace_strategy` to `stop_then_start` when volumes are declared and no explicit strategy was chosen, **and** the reconcile loop applies the same fallback at read time for any deployment with volume_refs whose config has no explicit strategy — so future config-edit paths can't reintroduce the two-writers-on-one-datadir hazard (postgres crash-loops on the postmaster lock; other stores risk corruption). An explicit `start_then_stop` choice is respected by both layers (legit for read-mostly shared caches). `removeContainer` already stops gracefully (30s) before removal, so databases get a clean shutdown.

**Frontend.** Create form: a Volumes editor (name + mountpoint rows, both sources, with the stop-then-start hint; half-filled rows are a form error instead of being silently dropped) and a Start command field parsed by the new quote-aware `parseCommandLine` (`packages/shared/src/command.ts` — UX-only tokenizer; the argv goes to the Engine API directly, no shell ever runs; unbalanced quotes surface as a form error). The client form imports it via the new `@hotbox/shared/command` subpath export — the barrel re-exports template-loader (node:fs), which would break the browser bundle at `next build`. Service detail: a Runtime section showing declared volumes (docker volume name computed via the shared `dockerVolumeName`) and command/entrypoint. `ServiceDetail['config']` is now typed from the zod schema (`ServiceConfigInput`, exported from @hotbox/shared) instead of a hand-mirrored interface.

### Deliberate limitations

- **No volume edit on an existing service yet** — config has no PATCH endpoint; declare volumes at create. (The derivation model + the reconciler's read-time replace-strategy fallback mean a future config editor gets correct behavior for free.)
- **No volume delete/GC** — `volumes` rows and docker volumes outlive service archive (archive intentionally preserves data). Manual `docker volume rm` after hard cleanup; the ownership check in `resolveVolumeRefs` refuses to silently adopt a leftover row on slug reuse.
- **No volume size limits/quotas**; `size_bytes_estimate` stays unpopulated.
- **Short DNS aliases still collide** on `hotbox-public` when two projects reuse a slug (pre-existing; rpc-proxy depends on `<slug>-erigon`, so the short aliases can't be dropped from shared networks yet). Mitigation: use the qualified alias for wiring. Existing containers pick the new alias up on their next redeploy.
- **One-off command overrides survive ingress edits** (verbatim-clone contract, same as one-off env) until the next plain redeploy — documented on the schema.

### Verification

- 140 unit tests pass (`pnpm test`), 31 new across schema validation (volumes/command incl. volumes-on-github accepted while `requires` stays blocked; template command/volume rejection; trailing-dash rejection; empty-string command elements), `parseCommandLine` tokenizer cases, `dockerVolumeName`/`qualifiedServiceName` namespacing (incl. the underscore anti-aliasing case), and `buildOptionsForRole` (mount-by-name, command passthrough, template-role isolation, qualified aliases + 63-char skip). Typecheck clean across 9 packages; `next build` clean (catches the client-bundle barrel-import class of failure that tsc/vitest don't).
- Live smoke (after control-plane update): create a postgres service with a `data` volume + `wal_level=logical` command → settings visible in logs, data survives Redeploy, replacement is stop-then-start; a github-sourced service with a volume gets populated volume_refs on its first build; Rebuild and ingress edits carry refs/command forward.

Files: `packages/shared/src/api.ts` (ShortNameSchema, VolumeMountSchema, CommandSchema, ServiceConfigSchema, ServiceConfigInput, CreateDeploymentInputSchema, template refine), `packages/shared/src/command.ts` + `packages/shared/src/naming.ts` (new) + subpath exports in `packages/shared/package.json`, `packages/shared/src/index.ts`, `packages/db/src/schema.ts` (ServiceConfig, VolumeRef.name), `apps/api/src/lib/volume-refs.ts` (new), `apps/api/src/routes/services.ts` (create/redeploy/redeployForConfigChange + stop_then_start default + template-override guard), `apps/api/src/build-worker.ts`, `apps/api/src/routes/projects.ts` (env duplicate), `apps/reconciler/src/template-runner.ts` (ensureDeploymentInfra, mount-by-name, command passthrough, qualified aliases), `apps/reconciler/src/loop.ts` (read-time replace-strategy fallback), `apps/web/src/lib/types.ts`, `apps/web/src/app/services/new/create-service-form.tsx` (volumes editor + start command + half-filled-row guard), `apps/web/src/app/services/[id]/page.tsx` (Runtime section). Tests: `packages/shared/tests/volumes-command.test.ts`, `packages/shared/tests/parse-command-line.test.ts`, `packages/shared/tests/naming.test.ts`, `apps/reconciler/tests/build-options.test.ts` (all new).

### Post-Phase-5 fix — persist `services.image_source` on create

`POST /api/services` never included `image_source` in the services insert, so the column silently kept its DB default `'image'` for github services. The build pipeline still ran (the route branches on the request body, not the column), but the service detail page hid the Builds panel and github_source entirely — a github service stuck in `pending` showed no explanation. Latent since Phase 4a; surfaced by the first live github deploy (EFP orchestrator, 2026-07-06). Fix: one line in the create route (`apps/api/src/routes/services.ts`). Rows created before the fix need a one-off data repair: `UPDATE services SET image_source = 'github' WHERE id = '<id>';`.

### eth-archive template — raise erigon eth_getLogs range limit

Erigon v3 defaults `--rpc.blockrange.limit` to 1000 blocks, which rejected the EFP indexer's 5000-block `eth_getLogs` batches ("query block range exceeds server limit, narrow your filter: 1000") and silently contradicted the rpc-proxy's public policy (`MAX_GETLOGS_BLOCK_RANGE = 10_000` in `apps/rpc-proxy/src/policy.ts` — public ranges of 1001–10000 passed the proxy and then failed at the node). Template now sets `--rpc.blockrange.limit=100000`: internal consumers (direct `:8545` on hotbox-public) get headroom for indexer backfills, while the public path stays capped at 10k by the proxy. Takes effect on the next Redeploy of the eth-archive service (template commands are read at reconcile but containers only recreate on a deployment version bump).

### GitHub builds — non-secret variables as docker build args

Build-time config (Next.js `NEXT_PUBLIC_*` and friends gets baked into client bundles at `next build`) previously had nowhere to live except committed files — publishing browser-exposed-but-scrapeable IDs (Alchemy/WalletConnect etc.) to public repos. Now the build worker resolves the service's variables before building and passes every key whose **winning value is non-secret** as docker build args (`plainVariablesOf` in `resolve-variables.ts`; filtering on the winner means a secret override removes the key even when a plain value sits underneath). Dockerfiles opt in per key with a matching `ARG`; undeclared keys just produce docker's "not consumed" warning. Secrets are never passed — build args can surface in build-stage layer history. The build log lists the arg *keys* (never values). The same resolution is reused for the deployment's env_snapshot so a concurrent variable edit can't produce a build/runtime mismatch. First consumer: the EFP frontend (`NEXT_PUBLIC_*` project IDs live as hotbox variables instead of in the repo).

### RPC ingress — CORS for browser JSON-RPC clients

Browser wallets/dapps (relics-app and efp-app wagmi transports pointed at the tokened RPC endpoint) preflight every JSON-RPC call — `Content-Type: application/json` POSTs are never "simple" requests — and nothing in the chain answered OPTIONS or attached `Access-Control-Allow-Origin`, so browsers blocked the responses outright (server-to-server callers were unaffected).

Fix lives entirely in the file provider (`infra/traefik/dynamic/dynamic.yml`), hot-reloaded by Traefik's `watch: true` — no container relabeling, which matters because the `hotbox-auth@file` reference is baked into the erigon container's immutable docker labels and relabeling would restart the archive node. `hotbox-auth` is now a `chain` middleware: `hotbox-rpc-cors` (Traefik `headers` CORS — answers preflights directly, before auth; rpc-proxy has no OPTIONS handling) followed by `hotbox-authz` (the previous forwardAuth, renamed). Allowed origins: the prod app domains (relics.bid/www, efp.app), localhost dev ports, and a regex for `*.on.hotbox.wtf` staging fronts — adding an origin is a one-line edit here. CORS is deliberately abuse-limiting only: the RPC token ships in the apps' public JS bundles, so the real controls remain per-token rate limits and the rpc-proxy method allowlist.
